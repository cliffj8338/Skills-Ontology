/**
 * src/views/reports.js — Reports Dashboard View
 * Phase 7a: Reports view extraction.
 *
 * Monolith source: L24603–24953
 *
 * Exports:
 *   initReports            — render reports view (demo or live)
 *   openDemoScoutingReport — iframe overlay for sample scouting reports
 *   viewDemoSampleReport   — maps current profile to a sample report
 *
 * Legacy globals consumed (still owned by legacy.js):
 *   userData, blueprintData, isReadOnlyProfile
 *   getVisibleRoles, closeExportModal
 *   showAuthModal, showReportFormatPicker, showScoutingReportPicker, exportBlueprint
 */
...
export function initReports() {
    ...
    var allSavedJobs = window.userData.savedJobs || [];
    var pipelineJobs = [];
    ...
    var allSkills = (window.blueprintData && window.blueprintData.skills) || [];
    var visibleRoles = (typeof window.getVisibleRoles === 'function') ? window.getVisibleRoles() : [];
    ...
    if (window.blueprintData) {
        completeness.outcomes = (window.blueprintData.outcomes || []).filter(function(o) { return o.shared; }).length > 0 ? 100 : 0;
        completeness.values = (window.blueprintData.values || []).filter(function(v) { return v.selected; }).length > 0 ? 100 : 0;
        completeness.purpose = window.blueprintData.purpose ? 100 : 0;
    }
    var userCerts = (window.blueprintData && window.blueprintData.certifications) || [];
    var userEdu = (window.blueprintData && window.blueprintData.education) || [];
...
export function openDemoScoutingReport(filePath, name, isMismatch) {
    ...
    var currentProfileName = (window.userData.profile && window.userData.profile.name) || '';
...
export function viewDemoSampleReport(format) {
    ...
    var tid = window.userData.templateId || '';
    ...
    var currentName = ((window.userData.profile && window.userData.profile.name) || '').toLowerCase();
