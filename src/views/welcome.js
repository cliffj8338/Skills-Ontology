// views/welcome.js — Blueprint v4.47.00
...
function wizardGetMarketBaseAnchor() {
    ...
    if (typeof window.calculateTotalMarketValue === 'function') {
        try {
            var mv2 = window.calculateTotalMarketValue('evidence');
            if (mv2 && mv2.marketRate && mv2.marketRate > 0) return mv2.marketRate;
        } catch (e) {}
    }
    var SALARY_TABLE = {
        education: [29120, 62340, 83010, 132550, 165820],
    ...
    return SALARY_TABLE[func][seniority];
}
...
function wizardAddGrowthSkill(growthIdx) {
    ...
    if (alreadyAdded) {
        showToast('"' + g.name + '" is already in your growth plan.', 'info', 2000);
        return;
    }
...
export function wizardSaveSkills() {
    ...
    if (wizardState.growthSkills && wizardState.growthSkills.length > 0) {
        ...
        if (window._userData) window._userData.growthSkills = wizardState.growthSkills;
    }
    wizardNext();
}
