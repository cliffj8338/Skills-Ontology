 ...
 * Phase 7b: Settings view extraction.
 *
 * Monolith source: L40727–42896
 *
 * Exports:
 ...
 *
 * Legacy globals consumed (still owned by legacy.js):
 *   userData, skillsData, blueprintData, fbUser, fbDb
 *   appContext, isReadOnlyProfile, currentPreset, sharePresets
 *   blueprintTab, adminSubTab, networkInitialized, networkMatchMode
 *   BLIND_FIELDS, saveAll, debouncedSave, readOnlyGuard
 *   formatWorkDate, proficiencyValue, registerInSkillLibrary
 *   findCertInLibrary, getCertSkillAssociations, renderPresetCard
 *   renderAdminOverview, getBlindDefaults, setBlindDefault
 *   filterByRole, setNetworkMatchMode, viewMyData, exportMyData
 *   requestDataDeletion, openEditSkillModal, confirmDeleteSkill
 *   handleLinkedInMerge, handleProfilePhoto, removeProfilePhoto
 *   saveSettings, sendFeedback
 *   closeExportModal, showAuthModal, rescoreAllJobs, refreshAllViews
 */
...
export function renderSettingsTabContent() {
    switch (window.currentSettingsTab) {
        ...
        case 'privacy':      return renderPrivacyAndData();
        default:             return renderProfileSettings();
    }
}
window.renderSettingsTabContent = renderSettingsTabContent;
        ...
        <div class="blueprint-section">
            ...
            <div class="settings-group">
                ...
                <input type="text" class="settings-input" id="settingName"
                       value="${escapeHtml(ud.profile.name || '')}"
                ...
                <div style="display:flex; align-items:center; gap:16px;">
                    <div id="settingPhotoPreview" onclick="document.getElementById('settingPhotoInput').click()"
                         style="width:64px; height:64px; border-radius:50%; background:var(--accent); color:#fff;
                                ...
                                ${ud.profile.photo ? 'padding:0' : ''}">
                        ${(ud.profile.name || 'U').split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase()}
                    </div>
                    <div>
                        ...
                        ${ud.profile.photo ? '<button onclick="removeProfilePhoto()" style="background:none; border:none; color:#ef4444; cursor:pointer; font-size:0.82em; margin-left:8px;">Remove</button>' : ''}
            ...
            <div class="settings-group">
                ...
                <input type="email" class="settings-input" id="settingEmail"
                       value="${escapeHtml(ud.profile.email || '')}"
            ...
            <div class="settings-group">
                ...
                <input type="tel" class="settings-input" id="settingPhone"
                       value="${escapeHtml(ud.profile.phone || '')}"
            ...
            <div class="settings-group">
                ...
                <input type="url" class="settings-input" id="settingLinkedIn"
                       value="${escapeHtml(ud.profile.linkedinUrl || '')}"
            ...
            <div class="settings-group">
                ...
                <select class="settings-select" id="settingLocation">
                    ${['Philadelphia, PA','San Francisco Bay Area, CA','New York City, NY','Seattle, WA','Boston, MA','Washington DC','Los Angeles, CA','San Diego, CA','Denver, CO','Austin, TX','Chicago, IL','Atlanta, GA','Remote','Phoenix, AZ','Dallas, TX','Houston, TX','Miami, FL','Portland, OR','Charlotte, NC','Nashville, TN','Indianapolis, IN','Columbus, OH','Kansas City, MO','Omaha, NE','Lincoln, NE','Other US Metro','Rural US'].map(function(loc) {
                        return '<option value="' + loc + '"' + (ud.profile.location === loc ? ' selected' : '') + '>' + loc + '</option>';
                    }).join('')}
                </select>
            ...
            <div class="settings-group">
                ...
                <div style="position:relative;">
                    ...
                    <input type="text" class="settings-input" id="settingComp"
                           value="${ud.profile.reportedComp ? Math.round(ud.profile.reportedComp).toLocaleString() : ''}"
...
export function refreshExperienceContent() {
    var target = document.getElementById('blueprintTabContent') || document.getElementById('settingsTabContent');
    if (target && (window.blueprintTab === 'experience' || window.currentSettingsTab === 'experience')) {
        target.innerHTML = renderExperienceSettings();
    }
}
window.refreshExperienceContent = refreshExperienceContent;
...
export function renderExperienceSettings() {
    ...
    var certItems = ud.certifications || [];
    var html = '';

    // LinkedIn Merge Import
    var lastMerge = (ud.importStats || {}).lastMerge;
    ...
    html += '<div class="blueprint-section">'
        ...
        + '<div class="coaching-tip-content">Positions at the same company are grouped to show your career trajectory. Multiple roles demonstrate growth, expanding scope, and increasing responsibility.</div>'
        + '</div>';
    ...
    if (whItems.length > 0) {
        groupArr.forEach(function(group) {
            ...
            if (isProgression) {
                ...
                html += '<div style="padding:16px; margin-bottom:12px; background:var(--c-surface-1); border:1px solid var(--c-surface-5b); border-radius:10px; border-left:3px solid var(--c-accent);">';
                html += '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">'
                    + '<div>'
                    + '<div style="font-weight:700; color:var(--c-accent); font-size:0.95em;">' + escapeHtml(coName) + '</div>'
                    + '<div style="color:var(--c-muted); font-size:0.78em;">' + window.formatWorkDate(firstStart) + ' \u2013 ' + (lastEnd === 'Present' ? 'Present' : window.formatWorkDate(lastEnd))
                    + (tenure > 0 ? ' \u00B7 ' + tenure + ' year' + (tenure !== 1 ? 's' : '') : '') + '</div>'
                    + '</div>'
                    + '<span style="font-size:0.68em; padding:3px 8px; border-radius:4px; background:rgba(96,165,250,0.1); color:#60a5fa; font-weight:600; letter-spacing:0.03em;">'
                    + '\u2191 ' + group.length + ' ROLES</span>'
                    + '</div>';
                var reversedGroup = group.slice().reverse();
                reversedGroup.forEach(function(entry, gIdx) {
                    var job = entry.job; var idx = entry.idx;
                    var isLatest = gIdx === 0; var isHidden = job.hidden === true;
                    var dateRange = window.formatWorkDate(job.startDate || '?') + ' \u2013 ' + (job.current ? 'Present' : window.formatWorkDate(job.endDate || '?'));
                    ...
                    html += '<div style="position:absolute; left:-5px; top:14px; width:8px; height:8px; border-radius:50%; background:' + (isLatest ? 'var(--c-accent)' : 'var(--c-surface-5b)') + ';"></div>';
                    html += '<div style="position:absolute; top:6px; right:4px; display:flex; align-items:center; gap:6px;">'
                        ...
                        + 'style="background:none; border:none; cursor:pointer; font-size:0.72em; padding:1px 4px; border-radius:3px; color:' + (isHidden ? '#f59e0b' : 'var(--c-muted)') + ';">'
                    ...
                    if (job.achievements && job.achievements.length > 0 && isLatest) {
                        ...
                        job.achievements.forEach(function(ach) {
                            html += '<div style="font-size:0.8em; color:var(--c-heading); padding:2px 0 2px 14px; position:relative; line-height:1.45;"><span style="position:absolute; left:0; color:var(--c-success);">\u25B8</span>' + escapeHtml(ach) + '</div>';
                        });
                        html += '</div>';
                    }
                    html += '</div>';
                ...
                var entry = group[0]; var job = entry.job; var idx = entry.idx;
                var isHidden = job.hidden === true;
                var dateRange = window.formatWorkDate(job.startDate || '?') + ' \u2013 ' + (job.current ? 'Present' : window.formatWorkDate(job.endDate || '?'));
                html += '<div id="wh-card-' + idx + '" style="padding:16px; margin-bottom:12px; background:var(--c-surface-1); border:1px solid var(--c-surface-5b); border-radius:10px; position:relative;' + (isHidden ? ' opacity:0.4;' : '') + '">';
                html += '<div style="position:absolute; top:10px; right:10px; display:flex; align-items:center; gap:6px;">'
                    + '<button onclick="toggleWorkHistoryHidden(' + idx + ')" title="' + (isHidden ? 'Show in Blueprint' : 'Hide from Blueprint') + '" '
                    + 'style="background:none; border:none; cursor:pointer; font-size:0.78em; padding:2px 6px; border-radius:4px; color:' + (isHidden ? '#f59e0b' : 'var(--c-muted)') + '; transition:all 0.2s;">'
                ...
                if (job.description) { html += '<div style="color:var(--c-heading); font-size:0.85em; margin-top:8px; line-height:1.5;">' + escapeHtml(job.description) + '</div>'; }
                if (job.achievements && job.achievements.length > 0) {
                    html += '<div style="margin-top:8px;">';
                    job.achievements.forEach(function(ach) {
                        html += '<div style="font-size:0.83em; color:var(--c-heading); padding:3px 0 3px 14px; position:relative; line-height:1.45;"><span style="position:absolute; left:0; color:var(--c-success);">\u25B8</span>' + escapeHtml(ach) + '</div>';
                    });
                    html += '</div>';
                }
                html += '</div>';
        ...
        html += '<div style="text-align:center; padding:24px; color:var(--c-faint); font-size:0.9em;">No work history added yet. Add your roles to power resume generation.</div>';
    }
    html += '<button onclick="addWorkHistoryItem()" style="background:var(--c-accent-bg-5a); border:1px solid var(--c-accent-border-4); color:var(--c-accent-deep); padding:10px 20px; border-radius:8px; cursor:pointer; font-size:0.9em; font-weight:600; margin-top:12px;">+ Add Position</button></div>';

    // Education
    html += '<div class="blueprint-section" style="margin-top:24px;">'
        + '<div class="blueprint-section-header"><div class="blueprint-section-title">'
        + '<span class="section-icon">' + bpIcon('graduation',20) + '</span><span>Education</span>'
        + '<span class="section-count">' + edItems.length + ' entries</span></div></div>';
    if (edItems.length > 0) {
        ...
        edItems.forEach(function(ed, idx) {
            ...
            var dateStr = '';
            if (ed.startYear && ed.endYear) { dateStr = ed.startYear + ' \u2013 ' + (ed.currentlyEnrolled ? 'Present' : ed.endYear); }
            else if (ed.startYear && ed.currentlyEnrolled) { dateStr = ed.startYear + ' \u2013 Present'; }
            else if (ed.year) { dateStr = ed.year; }
            html += '<div style="padding:12px 16px; margin-bottom:8px; background:var(--c-surface-1); border:1px solid var(--c-surface-5b); border-radius:8px;">'
                + '<div style="display:flex; justify-content:space-between; align-items:flex-start;">'
                + '<div style="flex:1; min-width:0;">'
                + '<div style="display:flex; align-items:center; gap:8px; margin-bottom:2px;"><span style="font-size:0.68em; padding:2px 8px; border-radius:10px; background:var(--c-surface-4a); color:var(--c-muted); font-weight:600; white-space:nowrap;">' + icon + ' ' + typeLabel + '</span></div>'
                + '<div style="font-weight:600; color:var(--c-text-alt); font-size:0.9em; margin-top:4px;">' + (ed.degree || '') + (ed.field ? ' in ' + ed.field : '') + '</div>'
                + '<div style="color:var(--c-muted); font-size:0.82em; margin-top:2px;">' + (ed.school || '') + (ed.issuingAuthority ? ' \u00B7 Issued by ' + ed.issuingAuthority : '') + (dateStr ? ' \u00B7 ' + dateStr : '') + '</div>'
                + (hasLinkedCreds ? '<div style="margin-top:6px;">' + ed.linkedCredentials.map(function(ci) {
                    var cert = ud.certifications[ci];
                    if (!cert) return '';
                    return '<span style="font-size:0.72em; padding:2px 8px; border-radius:8px; background:var(--c-accent-bg-5a); color:var(--c-accent-deep); font-weight:600;">\uD83D\uDD17 ' + escapeHtml(cert.name + (cert.abbr ? ' (' + cert.abbr + ')' : '')) + '</span> ';
                }).join('') + '</div>' : '')
        ...
        html += '<div style="text-align:center; padding:16px; color:var(--c-faint); font-size:0.88em;">No education entries yet.</div>';
    }
    html += '<button onclick="addEducationItem()" style="background:var(--c-accent-bg-5a); border:1px solid var(--c-accent-border-4); color:var(--c-accent-deep); padding:8px 16px; border-radius:8px; cursor:pointer; font-size:0.85em; font-weight:600; margin-top:8px;">+ Add Education</button></div>';

    // Certifications
    html += '<div class="blueprint-section" style="margin-top:24px;">'
        + '<div class="blueprint-section-header"><div class="blueprint-section-title">'
        + '<span class="section-icon">' + bpIcon('award',20) + '</span><span>Certifications, Licenses & Degrees</span>'
        + '<span class="section-count">' + certItems.length + ' entries</span></div></div>';
    if (certItems.length > 0) {
        certItems.forEach(function(cert, idx) {
            ...
            var libSkills = cert.libraryMatch ? (window.findCertInLibrary(cert.libraryMatch) || {}).skills : null;
            var totalLinks = linkedCount + (libSkills ? libSkills.length : 0);
            html += '<div style="padding:12px 16px; margin-bottom:8px; background:var(--c-surface-1); border:1px solid var(--c-surface-5b); border-radius:8px; display:flex; justify-content:space-between; align-items:center;">'
                + '<div>'
                + '<div style="font-weight:600; color:var(--c-text-alt); font-size:0.9em;">' + (cert.name || '') + (cert.abbr ? ' <span style="color:var(--c-accent); font-size:0.85em;">(' + cert.abbr + ')</span>' : '') + '</div>'
                + '<div style="display:flex; gap:6px; align-items:center; margin-top:3px;">'
                + '<span style="color:var(--c-muted); font-size:0.82em;">' + (cert.issuer || '') + (cert.year ? ' \u00B7 ' + cert.year : '') + '</span>'
        ...
        html += '<div style="text-align:center; padding:16px; color:var(--c-faint); font-size:0.88em;">No credentials yet. Add certifications, licenses, or degrees from the library.</div>';
    }
    html += '<button onclick="addCertItem()" style="background:var(--c-accent-bg-5a); border:1px solid var(--c-accent-border-4); color:var(--c-accent-deep); padding:8px 16px; border-radius:8px; cursor:pointer; font-size:0.85em; font-weight:600; margin-top:8px;">+ Add Credential</button></div>';

    html += '<div style="margin-top:24px; display:flex; gap:12px;">'
        + '<button onclick="saveAll(); showToast(\'Experience saved.\', \'success\')" style="background:var(--accent); color:#fff; border:none; padding:12px 28px; border-radius:8px; cursor:pointer; font-weight:600; font-size:0.95em;">Save All Changes</button>'
        + '</div>';
    return html;
}
window.renderExperienceSettings = renderExperienceSettings;

// ─── Work History CRUD ────────────────────────────────────────────────────────

export function addWorkHistoryItem() { if (window.readOnlyGuard()) return; openWorkHistoryModal(-1); }
export function editWorkHistoryItem(idx) { if (window.readOnlyGuard()) return; openWorkHistoryModal(idx); }
export function removeWorkHistoryItem(idx) {
    if (window.readOnlyGuard()) return;
    if (!confirm('Remove this position?')) return;
    var ud = window.userData; var sd = window.skillsData;
    var removedJob = ud.workHistory[idx];
    ud.workHistory.splice(idx, 1);
    if (removedJob) {
        var removedTitle = (removedJob.title || '').toLowerCase().trim();
        var stillExists = ud.workHistory.some(function(j) { return (j.title || '').toLowerCase().trim() === removedTitle; });
        if (!stillExists && removedTitle) {
            var removedRoleIds = new Set(); removedRoleIds.add(removedTitle);
            [sd.roles, ud.roles].forEach(function(rolesArr) {
                ...
                for (var ri = rolesArr.length - 1; ri >= 0; ri--) {
                    ...
                    if (rn === removedTitle || rid === removedTitle) { removedRoleIds.add(rn); removedRoleIds.add(rid); rolesArr.splice(ri, 1); }
                }
            });
            (sd.skills || []).forEach(function(s) {
                if (s.roles && Array.isArray(s.roles)) { s.roles = s.roles.filter(function(r) { return !removedRoleIds.has((typeof r === 'string' ? r : '').toLowerCase().trim()); }); }
            });
        }
    }
    window.saveAll(); if (window.fbUser) window.debouncedSave();
...
export function toggleWorkHistoryHidden(idx) {
    if (window.readOnlyGuard()) return;
    var job = (window.userData.workHistory || [])[idx];
    if (!job) return;
...
export function getVisibleRoles() {
    ...
    wh.forEach(function(job) { if (!job.hidden) visibleTitles.add((job.title || '').toLowerCase().trim()); });
    if (visibleTitles.size === 0) return [];
    var filtered = (sd.roles || []).filter(function(role) {
        var rName = (role.name || '').toLowerCase().trim();
        var rId = (role.id || '').toLowerCase().trim();
        return visibleTitles.has(rName) || visibleTitles.has(rId);
    });
    return filtered.length > 0 ? filtered : (sd.roles || []).slice();
}
window.getVisibleRoles = getVisibleRoles;

export function hideRoleFromNetwork(roleName) {
    if (window.readOnlyGuard()) return;
    var sd = window.skillsData; var ud = window.userData;
    var rLower = (roleName || '').toLowerCase().trim();
    var roleObj = (sd.roles || []).find(function(r) {
        return (r.name || '').toLowerCase().trim() === rLower || (r.id || '').toLowerCase().trim() === rLower;
    });
    var matchNames = new Set(); matchNames.add(rLower);
    if (roleObj) { matchNames.add((roleObj.name || '').toLowerCase().trim()); matchNames.add((roleObj.id || '').toLowerCase().trim()); }
    var found = false;
    (ud.workHistory || []).forEach(function(job) {
        var titleLower = (job.title || '').toLowerCase().trim();
        if (!job.hidden && matchNames.has(titleLower)) { job.hidden = true; found = true; }
    });
    if (!found) {
        (ud.workHistory || []).forEach(function(job) {
            ...
            matchNames.forEach(function(mn) {
                if (!found && mn && titleLower && (mn.indexOf(titleLower) !== -1 || titleLower.indexOf(mn) !== -1)) { job.hidden = true; found = true; }
            });
        });
    }
    if (found) {
        ...
        window.filterByRole('all');
        if (typeof window.setNetworkMatchMode === 'function') window.setNetworkMatchMode(window.networkMatchMode || 'you');
        showToast('Role hidden from Blueprint. Unhide it in Experience tab.', 'info', 3500);
    } else {
        var orphanIdx = (sd.roles || []).findIndex(function(r) {
            return matchNames.has((r.name || '').toLowerCase().trim()) || matchNames.has((r.id || '').toLowerCase().trim());
        });
        if (orphanIdx !== -1) {
            var orphanRole = sd.roles[orphanIdx];
            var orphanId = orphanRole.id; var orphanName = orphanRole.name;
            sd.roles.splice(orphanIdx, 1);
            if (ud.roles) { ud.roles = ud.roles.filter(function(r) { return (r.name || '').toLowerCase().trim() !== (orphanName || '').toLowerCase().trim() && (r.id || '').toLowerCase().trim() !== (orphanId || '').toLowerCase().trim(); }); }
            (sd.skills || []).forEach(function(s) { if (s.roles) { s.roles = s.roles.filter(function(rid) { return rid !== orphanId && rid !== orphanName; }); } });
            window.saveAll(); if (window.fbUser) window.debouncedSave();
            window.networkInitialized = false;
            var ric2 = document.getElementById('roleInfoCard'); if (ric2) ric2.style.display = 'none';
            window.filterByRole('all');
            if (typeof window.setNetworkMatchMode === 'function') window.setNetworkMatchMode(window.networkMatchMode || 'you');
...
window.hideRoleFromNetwork = hideRoleFromNetwork;

export function cleanOrphanRoles() {
    if (window.readOnlyGuard()) return;
    var sd = window.skillsData; var ud = window.userData;
    var whTitles = new Set();
    (ud.workHistory || []).forEach(function(j) { whTitles.add((j.title || '').toLowerCase().trim()); });
    var orphans = [];
    (sd.roles || []).forEach(function(r) {
        var rn = (r.name || '').toLowerCase().trim(); var ri = (r.id || '').toLowerCase().trim();
        if (!whTitles.has(rn) && !whTitles.has(ri)) orphans.push(r);
    });
    if (orphans.length === 0) { showToast('No orphan roles found.', 'info'); return; }
    orphans.forEach(function(orphan) {
        var oid = orphan.id; var oname = orphan.name;
        sd.roles = (sd.roles || []).filter(function(r) { return r !== orphan; });
        if (ud.roles) { ud.roles = ud.roles.filter(function(r) { return (r.name || '').toLowerCase().trim() !== (oname || '').toLowerCase().trim() && (r.id || '').toLowerCase().trim() !== (oid || '').toLowerCase().trim(); }); }
        (sd.skills || []).forEach(function(s) { if (s.roles) { s.roles = s.roles.filter(function(rid) { return rid !== oid && rid !== oname; }); } });
    });
    window.saveAll(); if (window.fbUser) window.debouncedSave();
    window.networkInitialized = false;
    ...
    if (typeof window.renderAdminOverview === 'function') {
        ...
        if (adminEl && window.adminSubTab === 'overview') window.renderAdminOverview(adminEl);
    }
}
window.cleanOrphanRoles = cleanOrphanRoles;
...
export function saveDevStats() {
    ...
    if (window.fbUser && window.fbDb) {
        window.fbDb.collection('users').doc(window.fbUser.uid).set({ devStats: ds }, { merge: true })
...
window.saveDevStats = saveDevStats;

// ─── openWorkHistoryModal ─────────────────────────────────────────────────────

export function openWorkHistoryModal(idx) {
    var ud = window.userData;
    var job = idx >= 0 ? ud.workHistory[idx] : { title:'', company:'', location:'', startDate:'', endDate:'', current:false, description:'', achievements:[] };
    var isEdit = idx >= 0;
    var modal = document.getElementById('exportModal');
    var mc = modal.querySelector('.modal-content');
    var achHTML = (job.achievements || []).map(function(a) {
        return '<div style="display:flex; gap:8px; margin-bottom:8px;">'
            + '<input type="text" class="settings-input wh-ach-input" value="' + escapeHtml(a||'') + '" placeholder="e.g., Increased pipeline velocity 40% through process redesign" style="flex:1; font-size:0.88em;">'
    ...
    mc.innerHTML = '<div class="modal-header">'
        + '<div class="modal-header-left"><h2 class="modal-title">' + (isEdit ? 'Edit Position' : 'Add Position') + '</h2>'
        + '<p style="color:var(--c-muted); margin-top:5px;">Add your role details and key accomplishments</p></div>'
        + '<button class="modal-close" aria-label="Close dialog" onclick="closeExportModal()">\u00D7</button></div>'
        + '<div class="modal-body" style="padding:24px; max-height:70vh; overflow-y:auto;">'
        + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">'
        + '<div class="settings-group"><label class="settings-label">Job Title *</label><input type="text" class="settings-input" id="whTitle" value="' + escapeHtml(job.title||'') + '" placeholder="VP of Strategy"></div>'
        + '<div class="settings-group"><label class="settings-label">Company *</label><input type="text" class="settings-input" id="whCompany" value="' + escapeHtml(job.company||'') + '" placeholder="Acme Corp"></div>'
        + '<div class="settings-group"><label class="settings-label">Location</label><input type="text" class="settings-input" id="whLocation" value="' + escapeHtml(job.location||'') + '" placeholder="Philadelphia, PA"></div>'
        + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">'
        + '<div class="settings-group"><label class="settings-label">Start Date</label><input type="text" class="settings-input" id="whStartDate" value="' + escapeHtml(job.startDate||'') + '" placeholder="Jan 2020"></div>'
        + '<div class="settings-group"><label class="settings-label">End Date</label><input type="text" class="settings-input" id="whEndDate" value="' + escapeHtml(job.endDate||'') + '" placeholder="Present" ' + (job.current ? 'disabled' : '') + '></div>'
        + '</div></div>'
        + '<div class="settings-group" style="margin-top:4px;"><label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:0.88em; color:var(--c-heading);">'
        + '<input type="checkbox" id="whCurrent" ' + (job.current ? 'checked' : '') + ' onchange="document.getElementById(\'whEndDate\').disabled=this.checked; if(this.checked) document.getElementById(\'whEndDate\').value=\'\';"> I currently work here</label></div>'
        + '<div class="settings-group" style="margin-top:10px;"><label class="settings-label">Role Description (Optional)</label>'
        + '<textarea class="settings-input" id="whDescription" rows="2" style="resize:vertical; font-size:0.88em;" placeholder="Brief summary of your responsibilities and scope">' + escapeHtml(job.description||'') + '</textarea></div>'
        + '<div class="settings-group" style="margin-top:10px;"><label class="settings-label">Key Achievements</label>'
        ...
        + '<button onclick="addAchievementInput()" style="background:none; border:1px dashed var(--c-accent-border-4); color:var(--c-accent); padding:6px 14px; border-radius:6px; cursor:pointer; font-size:0.82em; margin-top:4px;">+ Add Achievement</button></div>'
        + '<div style="display:flex; gap:10px; margin-top:20px;">'
        + '<button onclick="saveWorkHistoryFromModal(' + idx + ')" style="background:var(--accent); color:#fff; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600;">' + (isEdit ? 'Save Changes' : 'Add Position') + '</button>'
...
window.openWorkHistoryModal = openWorkHistoryModal;
...
export function saveWorkHistoryFromModal(idx) {
    ...
    var achievements = [];
    document.querySelectorAll('.wh-ach-input').forEach(function(inp) { var v = inp.value.trim(); if (v) achievements.push(v); });
    var ud = window.userData;
    var entry = {
        id: idx >= 0 ? (ud.workHistory[idx].id || 'wh-' + Date.now()) : 'wh-' + Date.now(),
    ...
    if (idx >= 0) { ud.workHistory[idx] = entry; } else { ud.workHistory.unshift(entry); }
    window.closeExportModal(); window.saveAll(); refreshExperienceContent();
...
export function addEducationItem() { if (window.readOnlyGuard()) return; openEducationModal(-1); }
export function editEducationItem(idx) { if (window.readOnlyGuard()) return; openEducationModal(idx); }
export function removeEducationItem(idx) {
    if (window.readOnlyGuard()) return;
    if (!confirm('Remove this education entry?')) return;
    window.userData.education.splice(idx, 1);
    window.saveAll(); refreshExperienceContent();
}
...
export function openEducationModal(idx) {
    var ud = window.userData;
    var ed = idx >= 0 ? ud.education[idx] : { school:'', degree:'', field:'', year:'', type:'degree', startYear:'', endYear:'', description:'', issuingAuthority:'', linkedCredentials:[], currentlyEnrolled:false };
    var isEdit = idx >= 0;
    var modal = document.getElementById('exportModal');
    var mc = modal.querySelector('.modal-content');
    var edTypes = {
    ...
    Object.keys(edTypes).forEach(function(key) {
        var t = edTypes[key]; var isActive = key === currentType;
    ...
    (ud.certifications || []).forEach(function(c, ci) {
        var name = c.name + (c.abbr ? ' (' + c.abbr + ')' : '');
        certOptions += '<option value="' + ci + '">' + escapeHtml(name) + '</option>';
    });
    mc.innerHTML = '<div class="modal-header">'
        + '<div class="modal-header-left"><h2 class="modal-title">' + (isEdit ? 'Edit Education' : 'Add Education') + '</h2>'
        + '<p style="color:var(--c-muted); margin-top:4px; font-size:0.85em;">All learning counts — degrees, licenses, trades, bootcamps, military</p></div>'
        + '<button class="modal-close" aria-label="Close dialog" onclick="closeExportModal()">\u00D7</button></div>'
        + '<div class="modal-body" style="padding:24px; max-height:75vh; overflow-y:auto;">'
        + pillsHtml
        + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">'
        + '<div class="settings-group"><label class="settings-label" id="edInstLabel">' + cfg.instLabel + '</label><input type="text" class="settings-input" id="edSchool" value="' + escapeHtml(ed.school||'') + '" placeholder=""></div>'
        + '<div class="settings-group"><label class="settings-label" id="edCredLabel">' + cfg.credLabel + '</label><input type="text" class="settings-input" id="edDegree" value="' + escapeHtml(ed.degree||'') + '" placeholder="' + cfg.credPlaceholder + '"></div>'
        + '</div>'
        + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-top:14px;">'
        + '<div class="settings-group"><label class="settings-label" id="edFieldLabel">' + cfg.fieldLabel + '</label><input type="text" class="settings-input" id="edField" value="' + escapeHtml(ed.field||'') + '" placeholder="' + cfg.fieldPlaceholder + '"></div>'
        + '<div class="settings-group" id="edAuthorityGroup" style="' + (cfg.showAuthority ? '' : 'display:none;') + '"><label class="settings-label">Issuing Authority</label><input type="text" class="settings-input" id="edAuthority" value="' + escapeHtml(ed.issuingAuthority||'') + '" placeholder="FAA, State Board, etc."></div>'
        + '<div class="settings-group" id="edYearSingleGroup" style="' + (cfg.showAuthority ? 'display:none;' : '') + '"><label class="settings-label">Year</label><input type="text" class="settings-input" id="edYearSingle" value="' + escapeHtml(ed.year||ed.endYear||'') + '" placeholder="2024"></div>'
        + '</div>'
        + '<div id="edDurationRow" style="display:' + (cfg.showDuration ? 'grid' : 'none') + '; grid-template-columns:1fr 1fr 1fr; gap:14px; margin-top:14px;">'
        + '<div class="settings-group"><label class="settings-label">Start Year</label><input type="text" class="settings-input" id="edStartYear" value="' + escapeHtml(ed.startYear||'') + '" placeholder="2004"></div>'
        + '<div class="settings-group"><label class="settings-label">End Year</label><input type="text" class="settings-input" id="edEndYear" value="' + escapeHtml(ed.endYear||ed.year||'') + '" placeholder="2006"></div>'
        + '<div class="settings-group" style="display:flex; align-items:flex-end;"><label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:0.82em; color:var(--c-muted); padding-bottom:10px;" id="edEnrolledLabel">'
        ...
        + '</div>'
        + '<div style="margin-top:14px;"><button onclick="var d=document.getElementById(\'edDescGroup\'); d.style.display=d.style.display===\'none\'?\'block\':\'none\';" style="background:none; border:none; color:var(--c-accent); font-size:0.82em; cursor:pointer; padding:0;">+ Add description or notes</button>'
        + '<div id="edDescGroup" class="settings-group" style="display:' + (ed.description ? 'block' : 'none') + '; margin-top:8px;">'
        + '<textarea class="settings-input" id="edDescription" rows="2" style="resize:vertical; min-height:50px;" placeholder="Key outcomes, honors, relevant details...">' + escapeHtml(ed.description||'') + '</textarea></div></div>'
        ...
        + '<div style="display:flex; gap:8px; align-items:center;"><select class="settings-select" id="edLinkedCred" style="flex:1; font-size:0.85em;">' + certOptions + '</select>'
        + '<button onclick="edAddNewCred()" style="padding:6px 12px; background:var(--c-accent-bg-5a); color:var(--c-accent-deep); border:1px solid var(--c-accent-border-4); border-radius:6px; cursor:pointer; font-size:0.78em; white-space:nowrap;">+ New</button></div>'
        + '<div id="edLinkedCredTags" style="display:flex; flex-wrap:wrap; gap:4px; margin-top:8px;"></div></div>'
        + '<div style="display:flex; gap:10px; margin-top:20px;">'
        + '<button onclick="saveEducationFromModal(' + idx + ')" style="background:var(--accent); color:#fff; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600;">' + (isEdit ? 'Save' : 'Add') + '</button>'
        + '<button onclick="closeExportModal()" style="background:transparent; color:var(--c-muted); border:1px solid var(--c-border-solid); padding:10px 20px; border-radius:8px; cursor:pointer;">Cancel</button>'
        + '</div></div>';
    modal.style.display = 'flex';
...
export function edSwitchType(newType) {
    ...
    var instLabel = document.getElementById('edInstLabel'); if (instLabel) instLabel.textContent = cfg.instLabel;
    var credLabel = document.getElementById('edCredLabel'); if (credLabel) credLabel.textContent = cfg.credLabel;
    var fieldLabel = document.getElementById('edFieldLabel'); if (fieldLabel) fieldLabel.textContent = cfg.fieldLabel;
    var degreeInput = document.getElementById('edDegree'); if (degreeInput) degreeInput.placeholder = cfg.credPlaceholder;
    var fieldInput = document.getElementById('edField'); if (fieldInput) fieldInput.placeholder = cfg.fieldPlaceholder;
    var authGroup = document.getElementById('edAuthorityGroup'); if (authGroup) authGroup.style.display = cfg.showAuthority ? '' : 'none';
    var yearSingle = document.getElementById('edYearSingleGroup'); if (yearSingle) yearSingle.style.display = cfg.showAuthority ? 'none' : '';
    var durRow = document.getElementById('edDurationRow'); if (durRow) durRow.style.display = cfg.showDuration ? 'grid' : 'none';
}
window.edSwitchType = edSwitchType;
...
window.edAddNewCred = edAddNewCred;
...
export function edRenderLinkedCredTags() {
    ...
    container.innerHTML = creds.map(function(ci, i) {
        var cert = window.userData.certifications[ci];
        var name = cert ? (cert.name + (cert.abbr ? ' (' + cert.abbr + ')' : '')) : 'Credential #' + ci;
        return '<span style="display:inline-flex; align-items:center; gap:4px; padding:3px 10px; background:var(--c-accent-bg-5a); color:var(--c-accent-deep); border-radius:12px; font-size:0.78em; font-weight:600;">'
            + '\uD83D\uDD17 ' + escapeHtml(name)
...
window.edRenderLinkedCredTags = edRenderLinkedCredTags;

export function edRemoveLinkedCred(i) {
    (window._edLinkedCreds || []).splice(i, 1);
    edRenderLinkedCredTags();
}
window.edRemoveLinkedCred = edRemoveLinkedCred;
...
export function saveEducationFromModal(idx) {
    ...
    if (credSelect && credSelect.value !== '') { var ci = parseInt(credSelect.value); if (!isNaN(ci) && linkedCreds.indexOf(ci) < 0) linkedCreds.push(ci); }
    var edType = window._edCurrentType || 'degree';
    var cfg = (window._edTypeConfigs || {})[edType] || {};
    var entry = {
        id: idx >= 0 ? (window.userData.education[idx].id || 'ed-' + Date.now()) : 'ed-' + Date.now(),
        type: edType,
    ...
    if (idx >= 0) { window.userData.education[idx] = entry; } else { window.userData.education.push(entry); }
    window.closeExportModal(); window.saveAll(); refreshExperienceContent();
...
export function addCertItem() { if (window.readOnlyGuard()) return; openCertModal(-1); }
export function editCertItem(idx) { if (window.readOnlyGuard()) return; openCertModal(idx); }
export function removeCertItem(idx) {
    if (window.readOnlyGuard()) return;
    if (!confirm('Remove this certification?')) return;
    window.userData.certifications.splice(idx, 1);
    window.saveAll(); refreshExperienceContent();
}
...
export function openCertModal(idx) {
    var ud = window.userData; var sd = window.skillsData;
    var cert = idx >= 0 ? ud.certifications[idx] : { name:'', abbr:'', issuer:'', year:'', type:'Certification', linkedSkills:[], tier:1, libraryMatch:null };
    var isEdit = idx >= 0;
    var modal = document.getElementById('exportModal');
    var mc = modal.querySelector('.modal-content');
    var linkedSkills = cert.linkedSkills || [];
    var linkedHTML = linkedSkills.map(function(sn) {
        return '<span style="display:inline-flex; align-items:center; gap:4px; padding:3px 10px; background:var(--c-purple-bg-1); color:var(--c-purple-light); border-radius:12px; font-size:0.82em; margin:2px 4px 2px 0;">'
    ...
    var skillOptions = (sd.skills || []).map(function(s) { return '<option value="' + s.name.replace(/"/g,'&quot;') + '">'; }).join('');
    var tierLevel = cert.tier || 1;
    var tierLabel = tierLevel >= 2 ? 'Advanced' : 'Proficient';
    var tierColor = tierLevel >= 2 ? '#f59e0b' : '#10b981';
    mc.innerHTML = '<div class="modal-header">'
        + '<div class="modal-header-left"><h2 class="modal-title">' + (isEdit ? 'Edit Credential' : 'Add Credential') + '</h2>'
        + '<p style="color:var(--c-muted); margin-top:4px; font-size:0.85em;">Search the library or enter manually</p></div>'
        + '<button class="modal-close" aria-label="Close dialog" onclick="closeExportModal()">\u00D7</button></div>'
        + '<div class="modal-body" style="padding:24px; max-height:75vh; overflow-y:auto;">'
        ...
        + '<input type="text" class="settings-input" id="certLibrarySearch" placeholder="Type name, abbreviation, or category..." oninput="onCertLibrarySearch()" autocomplete="off" style="font-size:0.9em;"' + (isEdit ? ' value="' + escapeHtml(cert.name||'') + '"' : '') + '>'
        + '<div id="certSearchResults" style="display:none; position:absolute; top:100%; left:0; right:0; z-index:100; max-height:240px; overflow-y:auto; background:var(--c-bg-solid-alt); border:1px solid var(--c-border-strong); border-radius:8px; margin-top:4px; box-shadow:0 8px 24px rgba(0,0,0,0.3);"></div></div>'
        ...
        + (tierLevel >= 2 ? '\u2B50 Tier 2: Advanced' : '\u2705 Tier 1: Proficient') + ' floor for linked skills</span></div>'
        + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">'
        + '<div class="settings-group" style="grid-column:1/-1;"><label class="settings-label">Credential Name *</label><input type="text" class="settings-input" id="certName" value="' + escapeHtml(cert.name||'') + '" placeholder="Full credential name"></div>'
        + '<div class="settings-group"><label class="settings-label">Abbreviation</label><input type="text" class="settings-input" id="certAbbr" value="' + escapeHtml(cert.abbr||'') + '" placeholder="PMP, CFA, CDL-A" style="font-size:0.9em;"></div>'
        + '<div class="settings-group"><label class="settings-label">Issuing Organization</label><input type="text" class="settings-input" id="certIssuer" value="' + escapeHtml(cert.issuer||'') + '" placeholder="PMI, SHRM, AWS, etc."></div>'
        + '<div class="settings-group"><label class="settings-label">Type</label><select class="settings-select" id="certType" style="font-size:0.88em;">'
        + ['Certification','License','Degree','Rating','Endorsement','Credential','Certificate','Commission'].map(function(t) { return '<option value="' + t + '"' + ((cert.type || 'Certification') === t ? ' selected' : '') + '>' + t + '</option>'; }).join('')
        + '</select></div>'
        + '<div class="settings-group"><label class="settings-label">Year Obtained</label><input type="text" class="settings-input" id="certYear" value="' + escapeHtml(cert.year||'') + '" placeholder="2023"></div></div>'
        + '<div class="settings-group" style="margin-top:14px;"><label class="settings-label">\uD83D\uDD17 Linked Skills</label>'
        ...
        + '<div style="display:flex; gap:8px;"><input type="text" class="settings-input" id="certLinkSkillInput" list="certSkillSuggestions" placeholder="Type skill name..." style="flex:1; font-size:0.88em;"><datalist id="certSkillSuggestions">' + skillOptions + '</datalist>'
        + '<button onclick="addCertLinkedSkill()" style="padding:6px 14px; background:var(--c-purple-bg-2a); color:var(--c-purple-light); border:1px solid var(--c-purple-border-1); border-radius:6px; cursor:pointer; font-size:0.82em; white-space:nowrap;">+ Link</button></div></div>'
        + '<div style="display:flex; gap:10px; margin-top:20px;">'
        + '<button onclick="saveCertFromModal(' + idx + ')" style="background:var(--accent); color:#fff; border:none; padding:10px 24px; border-radius:8px; cursor:pointer; font-weight:600;">' + (isEdit ? 'Save' : 'Add Credential') + '</button>'
        + '<button onclick="closeExportModal()" style="background:transparent; color:var(--c-muted); border:1px solid var(--c-border-solid); padding:10px 20px; border-radius:8px; cursor:pointer;">Cancel</button>'
        + '</div></div>';
    modal.style.display = 'flex';
...
export function onCertLibrarySearch() {
    ...
    matches.forEach(function(c) {
        ...
        html += '<div onclick="selectCertFromLibrary(\'' + escapeHtml(c.abbr).replace(/'/g,"&#39;") + '\')" '
            + 'style="padding:10px 16px; cursor:pointer; border-bottom:1px solid var(--c-surface-3a); transition:background 0.1s;" '
            + 'onmouseover="this.style.background=\'var(--c-surface-4a)\'" onmouseout="this.style.background=\'transparent\'">'
            + '<div style="display:flex; justify-content:space-between; align-items:center;"><div>'
            + '<div style="font-weight:600; color:var(--c-text-alt); font-size:0.88em;">' + escapeHtml(c.name) + ' <span style="color:var(--c-accent); font-size:0.85em;">(' + escapeHtml(c.abbr) + ')</span></div>'
            + '<div style="font-size:0.78em; color:var(--c-muted);">' + escapeHtml(c.category) + ' \u00B7 ' + escapeHtml(c.type) + (c.skills ? ' \u00B7 ' + c.skills.length + ' skill links' : '') + '</div>'
            + '</div><span style="font-size:0.7em; padding:2px 8px; border-radius:8px; background:' + tierBg + '; color:' + tierColor + '; font-weight:700;">' + tierTag + '</span>'
            + '</div></div>';
    });
...
export function selectCertFromLibrary(abbr) {
    var cert = window.findCertInLibrary(abbr);
    if (!cert) return;
    ...
    if (typeSelect) { var opts = Array.from(typeSelect.options); var match = opts.find(function(o) { return o.value === cert.type; }); if (match) typeSelect.value = cert.type; }
    var badge = document.getElementById('certTierBadge');
    var floorLabel = document.getElementById('certFloorLabel');
    if (badge) {
        var tColor = cert.tier >= 2 ? '#f59e0b' : '#10b981'; var tBg = cert.tier >= 2 ? '245,158,11' : '16,185,129';
        badge.style.display = '';
    ...
    var associations = window.getCertSkillAssociations(cert);
    if (associations.length > 0) {
        window._certLinkedSkills = [];
        var container = document.getElementById('certLinkedSkillsContainer'); container.innerHTML = '';
...
export function saveCertFromModal(idx) {
    ...
    var entry = {
        id: idx >= 0 ? (ud.certifications[idx].id || 'cert-' + Date.now()) : 'cert-' + Date.now(),
    ...
    if (idx >= 0) { ud.certifications[idx] = entry; } else { ud.certifications.push(entry); }

    var certFloorLevel = entry.tier >= 2 ? 'Advanced' : 'Proficient';
    var certFloorVal = window.proficiencyValue(certFloorLevel);
    var allLinkedSkills = entry.linkedSkills.slice();
    if (libCert && libCert.skills) { libCert.skills.forEach(function(sn) { if (allLinkedSkills.indexOf(sn) < 0) allLinkedSkills.push(sn); }); }
    var added = []; var bumped = [];
    var defaultRoles = (ud.roles || []).map(function(r) { return r.id; });
    allLinkedSkills.forEach(function(skillName) {
        var existing = ud.skills.find(function(s) { return s.name.toLowerCase() === skillName.toLowerCase(); });
        var existingSD = (sd.skills || []).find(function(s) { return s.name.toLowerCase() === skillName.toLowerCase(); });
        if (!existing) {
            var newSkill = { name: skillName, category: 'unique', level: certFloorLevel, roles: defaultRoles, key: false, addedFrom: 'certification', certSource: entry.name };
            ud.skills.push(newSkill); sd.skills.push(newSkill);
            if (typeof window.registerInSkillLibrary === 'function') window.registerInSkillLibrary(skillName, 'unique');
            added.push(skillName);
        } else {
            var currentVal = window.proficiencyValue(existing.level);
            if (currentVal < certFloorVal) { var oldLevel = existing.level; existing.level = certFloorLevel; if (existingSD) existingSD.level = certFloorLevel; bumped.push(skillName + ' (' + oldLevel + ' \u2192 ' + certFloorLevel + ')'); }
        }
    });
    window.closeExportModal(); window.saveAll(); window.rescoreAllJobs(); window.refreshAllViews(); refreshExperienceContent();
    if (added.length > 0 || bumped.length > 0) {
        showCertSkillNotification(entry, added, bumped, certFloorLevel);
    } else {
        showToast((idx >= 0 ? '\uD83C\uDFC5 Credential updated: ' : '\uD83C\uDFC5 Credential added: ') + entry.name, 'success');
    }
}
...
export function showCertSkillNotification(cert, added, bumped, floorLevel) {
    ...
    var html = '<div class="modal-header"><div class="modal-header-left"><h2 class="modal-title">\uD83C\uDFC5 Credential Impact</h2>'
        + '<p style="color:var(--c-muted); margin-top:4px; font-size:0.85em;">' + cert.name + (cert.abbr ? ' (' + cert.abbr + ')' : '') + '</p></div>'
        + '<button class="modal-close" aria-label="Close dialog" onclick="closeExportModal()">\u00D7</button></div>'
        + '<div class="modal-body" style="padding:24px;">'
        + '<div style="background:var(--c-green-bg-2a); border:1px solid var(--c-green-bg-6); border-radius:10px; padding:16px; margin-bottom:16px;">'
        + '<div style="font-weight:700; color:' + tierColor + '; font-size:0.9em; margin-bottom:8px;">' + tierLabel + ' \u2014 ' + floorLevel + ' floor applied</div>'
        + '<div style="color:var(--c-label); font-size:0.85em; line-height:1.5;">This credential proves competence in the skills below. Evidence points and market valuation have been updated automatically.</div></div>';
    if (added.length > 0) {
        html += '<div style="margin-bottom:14px;"><div style="font-weight:600; color:var(--c-success); font-size:0.85em; margin-bottom:8px;">\u2795 Skills Added to Profile (' + added.length + ')</div>';
        added.forEach(function(sn) { html += '<div style="padding:6px 12px; margin-bottom:4px; background:var(--c-green-bg-1a); border-radius:6px; font-size:0.85em; color:var(--c-text-alt);">' + sn + ' <span style="color:' + tierColor + '; font-size:0.85em;">\u2192 ' + floorLevel + '</span></div>'; });
        html += '</div>';
    }
    if (bumped.length > 0) {
        html += '<div style="margin-bottom:14px;"><div style="font-weight:600; color:var(--c-accent); font-size:0.85em; margin-bottom:8px;">\u2B06 Skills Upgraded (' + bumped.length + ')</div>';
        bumped.forEach(function(desc) { html += '<div style="padding:6px 12px; margin-bottom:4px; background:var(--c-accent-bg-2d); border-radius:6px; font-size:0.85em; color:var(--c-text-alt);">' + desc + '</div>'; });
        html += '</div>';
    }
    html += '<div style="text-align:center; margin-top:18px;"><button onclick="closeExportModal()" style="padding:10px 28px; background:var(--accent); color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:600;">Got It</button></div></div>';
    mc.innerHTML = html; modal.style.display = 'flex';
}
...
export function renderJobPreferences() {
    ...
    return `
        <div class="blueprint-section">
            ...
            <div class="coaching-tip">
                <div class="coaching-tip-title">${bpIcon("lightbulb",14)} HOW JOB MATCHING WORKS</div>
                <div class="coaching-tip-content">These settings control which jobs appear in your Opportunities feed. Set your seniority level to filter out irrelevant positions.</div>
            </div>
            <div class="settings-group">
                <label class="settings-label">Seniority Level</label>
                <select class="settings-select" id="settingSeniority">
                    ${['Entry','Mid','Senior','Executive'].map(function(v) {
                        var labels = {Entry:'Entry Level',Mid:'Mid Level',Senior:'Senior Level',Executive:'Executive Level'};
                        return '<option value="' + v + '"' + (ud.preferences.seniorityLevel === v ? ' selected' : '') + '>' + labels[v] + '</option>';
                    }).join('')}
                </select>
                <div class="settings-help"><strong>Entry:</strong> Includes junior, associate roles<br><strong>Mid:</strong> Includes senior, lead, manager roles<br><strong>Senior:</strong> Includes senior manager, director roles<br><strong>Executive:</strong> VP, Chief, Head of roles only</div>
            </div>
            ...
            <div class="settings-group">
                ...
                <input type="number" class="settings-input" id="settingMinSalary"
                       value="${ud.preferences.minSalary || ''}"
...
window.renderJobPreferences = renderJobPreferences;
...
export function renderDataExport() {
    return `
        <div class="blueprint-section">
            ...
            <div class="coaching-tip">
                <div class="coaching-tip-title">${bpIcon("lightbulb",14)} DATA PORTABILITY</div>
                <div class="coaching-tip-content">Export your complete Blueprint as JSON to back up all your data. Import from a previous export to restore or transfer to another device.</div>
            </div>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                <div style="background: rgba(96, 165, 250, 0.1); border: 1px solid rgba(96, 165, 250, 0.3); border-radius: 8px; padding: 20px;">
                    ...
                    <p style="color: #9ca3af; font-size: 0.9em; margin-bottom: 15px;">Download complete backup including skills, outcomes, values, and all settings</p>
                    <button class="export-btn-large" onclick="exportFullProfile()" style="width: 100%;">Download Backup</button>
                </div>
                <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 20px; ${window.isReadOnlyProfile ? 'display:none;' : ''}">
                    ...
                    <p style="color: #9ca3af; font-size: 0.9em; margin-bottom: 15px;">Restore from a previous backup or transfer from another device</p>
                    <input type="file" id="importFileInput" accept=".json" style="display: none;" onchange="importFullProfile(this)">
                    <button class="export-btn-large" onclick="document.getElementById('importFileInput').click()" style="width: 100%; background: #10b981;">Select Backup File</button>
                </div>
            </div>
            <div style="margin-top: 20px; padding: 15px; background: rgba(239, 68, 68, 0.1); border-left: 3px solid #ef4444; border-radius: 4px; ${window.isReadOnlyProfile ? 'display:none;' : ''}">
                ...
                <span style="color: #d1d5db; font-size: 0.9em;">Importing will replace all current data. Make sure to export first if you want to keep your current profile!</span>
            </div>
        </div>
    `;
}
...
export function renderPrivacyAndData() {
    ...
    // Sharing Presets
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('share',20) + '</span><span>Sharing Presets</span></div></div>'
        + '<div class="coaching-tip"><div class="coaching-tip-title">' + bpIcon('lightbulb',14) + ' CHOOSE YOUR SHARING LEVEL</div><div class="coaching-tip-content">Select a preset that matches your audience. These control what data appears in exports and shared views.</div></div>'
        + '<div class="values-grid">'
        + Object.entries(window.sharePresets).map(function(entry) { return window.renderPresetCard(entry[0], entry[1]); }).join('')
        + '</div>'
        + (window.currentPreset === 'custom' ? '<div style="margin-top:12px; padding:10px 14px; background:rgba(96,165,250,0.08); border-left:3px solid #60a5fa; border-radius:4px; font-size:0.85em; color:var(--c-heading);">' + bpIcon('info',12) + ' Custom mode active. Manage individual share toggles for outcomes and values in the <a onclick="switchView(\'blueprint\');" style="color:#60a5fa; cursor:pointer; text-decoration:underline;">Blueprint</a> tab. Skills are included based on your profile data.</div>' : '')
        + '</div>';

    // What You're Sharing
    var sd = window.skillsData; var bd = window.blueprintData;
    var skillCount = (sd.skills || []).length;
    var sharedSkills = window.currentPreset === 'full' ? skillCount :
        window.currentPreset === 'executive' ? Math.min(20, skillCount) :
        window.currentPreset === 'advisory' ? sd.skills.filter(function(s) { return s.key; }).length :
        window.currentPreset === 'board' ? sd.skills.filter(function(s) { return s.key || s.level === 'Mastery'; }).length :
        skillCount;
    var totalOutcomes = (bd.outcomes || []).length;
    var sharedOutcomes = bd.outcomes.filter(function(o) { return o.shared; }).length;
    var totalValues = (bd.values || []).length;
    var sharedValues = bd.values.filter(function(v) { return v.selected; }).length;
    function sharingLabel(shared, total) {
    ...
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('eye',20) + '</span><span>What You\'re Sharing</span></div></div>'
        + '<div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:16px;">'
        + '<div style="background:rgba(96,165,250,0.1); border:1px solid rgba(96,165,250,0.3); border-radius:8px; padding:20px; text-align:center;"><div style="font-size:2em; color:#60a5fa; font-weight:bold;">' + sharedSkills + '</div><div style="color:' + sc + '; margin-top:5px;">Skills</div>' + (sharedSkills < skillCount ? '<div style="font-size:0.72em; color:' + sc + '; margin-top:4px;">of ' + skillCount + ' total</div>' : '') + '</div>'
        + '<div style="background:rgba(16,185,129,0.1); border:1px solid rgba(16,185,129,0.3); border-radius:8px; padding:20px; text-align:center;"><div style="font-size:2em; color:' + (totalOutcomes === 0 ? '#64748b' : '#10b981') + '; font-weight:bold;">' + sharedOutcomes + '</div><div style="color:' + sc + '; margin-top:5px;">Outcomes</div>' + sharingLabel(sharedOutcomes, totalOutcomes) + '</div>'
        + '<div style="background:rgba(251,191,36,0.1); border:1px solid rgba(251,191,36,0.3); border-radius:8px; padding:20px; text-align:center;"><div style="font-size:2em; color:' + (totalValues === 0 ? '#64748b' : '#fbbf24') + '; font-weight:bold;">' + sharedValues + '</div><div style="color:' + sc + '; margin-top:5px;">Values</div>' + sharingLabel(sharedValues, totalValues) + '</div>'
        + '</div>'
        + '<div style="margin-top:16px; padding:12px 15px; background:rgba(251,191,36,0.08); border-left:3px solid #fbbf24; border-radius:4px; font-size:0.9em;"><strong style="color:#fbbf24;">Note:</strong> <span style="color:' + tc + ';">Sensitive content (recovery, personal loss) is flagged in Blueprint. Review before sharing.</span></div>'
        + '</div>';
    ...
    // Blind Mode Defaults
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('privacy',20) + '</span><span>Blind Mode Defaults</span></div></div>'
        + '<div style="color:' + tc + '; font-size:0.9em; line-height:1.6; margin-bottom:14px;">Control what identifying information is hidden in scouting reports by default. These settings apply globally. You can override them per report at generation time.</div>';
    var blindDefs = window.getBlindDefaults();
    (window.BLIND_FIELDS || []).forEach(function(f) {
        var isOn = blindDefs[f.key];
        html += '<div style="display:flex; align-items:center; justify-content:space-between; padding:12px 0; border-bottom:1px solid var(--c-border-subtle, rgba(255,255,255,0.06));"><div style="flex:1; min-width:0;"><div style="font-weight:500; font-size:0.9em;">' + f.label + '</div><div style="font-size:0.75em; color:' + sc + '; margin-top:2px;">' + f.desc + '</div></div>'
            + '<div class="rpt-toggle' + (isOn ? ' on' : '') + '" onclick="this.classList.toggle(\'on\'); setBlindDefault(\'' + f.key + '\', this.classList.contains(\'on\'));"></div></div>';
    });
    html += '</div>';

    // Privacy Activity Log
    var privLog = (window.userData.privacyLog || []).slice().reverse();
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('clock',20) + '</span><span>Privacy Activity</span></div></div>'
        + '<div style="color:' + tc + '; font-size:0.88em; line-height:1.5; margin-bottom:14px;">Audit trail of what privacy settings were active for each scouting report and share event.</div>';
    if (privLog.length > 0) {
        html += '<div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:0.82em;"><thead><tr>'
        ...
        privLog.slice(0, 50).forEach(function(entry) {
            ...
            Object.keys(blindKeys).forEach(function(k) { if (blindKeys[k]) { blindPills += '<span style="display:inline-block; font-size:0.7em; padding:1px 5px; border-radius:4px; background:rgba(16,185,129,0.12); color:#10b981; margin:1px 2px; font-weight:600;">' + (blindKeyLabels[k] || k) + '</span>'; } });
        ...
        if (privLog.length > 50) { html += '<div style="text-align:center; padding:8px; font-size:0.78em; color:' + sc + ';">Showing 50 of ' + privLog.length + ' entries</div>'; }
    } else {
        html += '<div style="padding:24px; text-align:center; color:' + sc + ';"><div style="font-size:1.4em; margin-bottom:8px; opacity:0.3;">' + bpIcon('clock', 32) + '</div><div style="font-weight:500;">No privacy events recorded yet</div><div style="font-size:0.82em; margin-top:4px;">Entries will appear here when you generate or share scouting reports.</div></div>';
    }
    html += '</div>';

    // Privacy & Legal
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('privacy',20) + '</span><span>Privacy & Data Rights</span></div></div>'
        + '<div style="color:' + tc + '; line-height:1.7;"><p style="margin-bottom:16px;">When signed in, your Blueprint data is stored securely in Firebase (Google Cloud). When not signed in, data persists locally in your browser. You have complete control over what you share.</p>'
        + '<div style="display:grid; gap:8px;">'
        + '<div style="padding:10px; background:' + bgCard + '; border-radius:6px;"><strong>CCPA</strong> &mdash; <span style="font-size:0.9em; color:' + sc + ';">Right to know, delete, and opt-out of data sale</span></div>'
        + '<div style="padding:10px; background:' + bgCard + '; border-radius:6px;"><strong>GDPR</strong> &mdash; <span style="font-size:0.9em; color:' + sc + ';">Right to access, rectify, erase, and port your data</span></div>'
        + '<div style="padding:10px; background:' + bgCard + '; border-radius:6px;"><strong>EEOC</strong> &mdash; <span style="font-size:0.9em; color:' + sc + ';">Employers must handle your data fairly, no discrimination</span></div>'
        + '</div></div></div>';

    // Data Rights Actions
    html += '<div class="blueprint-section"><div class="blueprint-section-header"><div class="blueprint-section-title"><span class="section-icon">' + bpIcon('lock',20) + '</span><span>Your Data Rights</span></div></div>'
        + '<div style="color:' + tc + '; line-height:1.7; font-size:0.93em;"><p style="margin-bottom:16px;">Under GDPR and similar privacy regulations, you have the right to access, export, and delete your personal data at any time.</p>'
        ...
        + '</div><div style="margin-top:12px; font-size:0.82em; color:var(--c-faint);">You must be signed in to use these features. Data deletion is permanent and cannot be undone.</div></div></div>';

    return html;
}
window.renderPrivacyAndData = renderPrivacyAndData;

// ─── disableBulkActionsInSampleMode ──────────────────────────────────────────
...
export function disableBulkActionsInSampleMode() {
    ...
    if (!isSample && typeof window.skillsData !== 'undefined' && window.skillsData.sample) isSample = true;
    if (isSample) {
        document.querySelectorAll('.bulk-action-btn').forEach(function(btn) {
            ...
            btn.style.pointerEvents = 'none'; btn.title = 'Not available in sample/demo mode';
        });
...
export function renderSkillsList() {
    ...
    let skills = window.userData.skills || [];
    if (categoryFilter !== 'all') { skills = skills.filter(s => s.category === categoryFilter); }
    if (searchQuery) { skills = skills.filter(s => s.name.toLowerCase().includes(searchQuery)); }
    if (skills.length === 0) {
        return `<div style="text-align: center; padding: 40px; color: #9ca3af;">No skills found. ${searchQuery ? 'Try a different search.' : 'Add some skills to get started!'}</div>`;
    }
    const grouped = {
    ...
    Object.entries(grouped).forEach(([category, categorySkills]) => {
        ...
        html += `<div style="margin-bottom: 25px;"><h3 style="color: ${label.color}; font-size: 1.1em; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;"><span>${label.name}</span><span style="font-size: 0.8em; opacity: 0.7;">(${categorySkills.length})</span></h3><div style="display: grid; gap: 10px;">`;
        categorySkills.forEach(skill => {
            const roleNames = skill.roles.map(roleId => {
                const role = window.userData.roles.find(r => r.id === roleId);
                return role ? role.name : roleId;
            }).join(', ');
            const levelColors = { 'Mastery':'#10b981','Expert':'#fb923c','Advanced':'#a78bfa','Proficient':'#60a5fa','Novice':'#94a3b8' };
            html += `<div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 15px;"><div style="display: flex; justify-content: space-between; align-items: start; gap: 15px;"><div style="flex: 1;"><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; flex-wrap: wrap;"><span style="font-weight: 600; font-size: 1.05em;">${escapeHtml(skill.name)}</span><span style="background: rgba(${category === 'skill' ? '59, 130, 246' : category === 'ability' ? '139, 92, 246' : category === 'workstyle' ? '16, 185, 129' : '251, 191, 36'}, 0.2); color: ${label.color}; font-size: 0.7em; padding: 2px 6px; border-radius: 3px;">${label.badge}</span>${skill.key ? '<span style="background: rgba(251, 191, 36, 0.2); color: #fbbf24; font-size: 0.7em; padding: 2px 6px; border-radius: 3px;">CORE</span>' : ''}</div><div style="color: #9ca3af; font-size: 0.9em;"><span style="color: ${levelColors[skill.level]}; font-weight: 600;">${escapeHtml(skill.level)}</span>${roleNames ? ` • ${escapeHtml(roleNames)}` : ''}</div></div><div style="display: flex; gap: 8px; flex-shrink: 0; ${window.isReadOnlyProfile ? 'display:none;' : ''}"><button onclick="openEditSkillModal('${escapeHtml(skill.name).replace(/'/g, "\\'")}', '${category}')" style="padding: 6px 12px; background: rgba(59, 130, 246, 0.2); color: #60a5fa; border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 4px; cursor: pointer; font-size: 0.9em; white-space: nowrap;">${bpIcon("edit",12)} Edit</button><button onclick="confirmDeleteSkill('${escapeHtml(skill.name).replace(/'/g, "\\'")}', '${category}')" style="padding: 6px 12px; background: rgba(239, 68, 68, 0.2); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 4px; cursor: pointer; font-size: 0.9em; white-space: nowrap;">${bpIcon("trash",14)}</button></div></div></div>`;
        });
        html += `</div></div>`;
    });
    return html;
...
export function exportFullProfile() {
    if (window.readOnlyGuard()) return;
    const ud = window.userData; const bd = window.blueprintData; const sd = window.skillsData;
    const fullData = {
        userData: ud,
        blueprintData: bd,
        skillsData: { skills: sd.skills, roles: sd.roles, skillDetails: sd.skillDetails },
        version: '1.0',
    ...
    link.href = url;
    link.download = `blueprint-full-${ud.profile.name || 'profile'}-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
    URL.revokeObjectURL(url);
}
window.exportFullProfile = exportFullProfile;
    ...
    reader.onload = function(e) {
        try {
            ...
            if (confirm('This will replace your current Blueprint data. Continue?')) {
                if (imported.userData) { var safeUserData = sanitizeImport(imported.userData); Object.assign(window.userData, safeUserData); }
                if (imported.blueprintData && typeof imported.blueprintData === 'object') {
                    if (Array.isArray(imported.blueprintData.values)) window.blueprintData.values = imported.blueprintData.values;
                    if (Array.isArray(imported.blueprintData.outcomes)) window.blueprintData.outcomes = imported.blueprintData.outcomes;
                    if (typeof imported.blueprintData.purpose === 'string') window.blueprintData.purpose = imported.blueprintData.purpose.slice(0, 5000);
                }
                if (imported.skillsData && typeof imported.skillsData === 'object') {
                    if (Array.isArray(imported.skillsData.skills)) window.skillsData.skills = imported.skillsData.skills;
                    if (Array.isArray(imported.skillsData.roles)) window.skillsData.roles = imported.skillsData.roles;
                    if (imported.skillsData.skillDetails) window.skillsData.skillDetails = imported.skillsData.skillDetails;
                }
                window.saveUserData();
