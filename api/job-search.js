// Blueprint™ Job Search Proxy — /api/job-search
// Aggregates: JSearch (RapidAPI), Remotive, USAJobs, Himalayas, Jobicy, Adzuna, The Muse
// Requires: RAPIDAPI_KEY env var for JSearch, USAJOBS_EMAIL + USAJOBS_API_KEY for USAJobs
// Renamed from jobs.js → job-search.js to eliminate naming collision with src/views/jobs.js
...
async function fetchJSearch(query, location, page, remoteOnly) {
  ...
  try {
    ...
    const jobs = (data.data || []).map(j => ({
      ...
      qualifications: (j.job_highlights && j.job_highlights.Qualifications || []).slice(0, 5),
      responsibilities: (j.job_highlights && j.job_highlights.Responsibilities || []).slice(0, 5)
    }));
...
async function fetchUSAJobs(query, location) {
  ...
  try {
    ...
    const data = await res.json();
    const results = (data && data.SearchResult && data.SearchResult.SearchResultItems) || [];
    const jobs = results.slice(0, 50).map(item => {
      const j = item.MatchedObjectDescriptor;
      const pos = (j.PositionLocation || [])[0] || {};
      const sal = (j.PositionRemuneration && j.PositionRemuneration[0]) || {};
      return {
        ...
        url: j.PositionURI || (j.ApplyURI && j.ApplyURI[0]) || '',
        ...
        description: (j.QualificationSummary || (j.UserArea && j.UserArea.Details && (j.UserArea.Details.MajorDuties || []).join(' ')) || '').substring(0, 3000),
...
async function fetchHimalayas(query) {
  try {
    ...
    if (!Array.isArray(data)) data = data.jobs || [];
...
export default async function handler(req, res) {
  ...
  // Top-level try/catch: guarantees all error paths return JSON, never a bare 500 HTML page
  try {
    const rawUrl = req.url || '/';
    const url = new URL(rawUrl, 'https://myblueprint.work');
    ...
    if (wantedSources.includes('themuse'))   fetchers.push(fetchMuse(query, category));
    ...
    // Catches URL parse errors, Promise.all failures, anything unexpected
    console.error('[job-search] Unhandled error:', e.message, e.stack);
    res.writeHead(500, headers);
    res.end(JSON.stringify({ error: 'Internal server error', detail: e.message, jobs: [], sources: {} }));
  }
};
