...
- Find Jobs sorting + entity fix (v4.44.62–64): Sort dropdown (Match % high/low, Title A–Z, Company A–Z, Location A–Z) defaults to highest match first; Find Jobs always shows quickScore consistently (no score swap on pipeline add); regex-based decodeHtmlEntities for numeric/named HTML entities from API responses; toast messages use decoded titles; addRemoteJobToPipeline uses displayed sort order via window._displayedJobs
