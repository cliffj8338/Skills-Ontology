...
- Find Jobs sorting + entity fix (v4.44.62–66): Sort dropdown (Match % high/low, Title A–Z, Company A–Z, Location A–Z) defaults to highest match first; Find Jobs always shows quickScore consistently; pipeline preserves quickScore on add (matchData.score = job.matchScore); regex-based decodeHtmlEntities decodes named entities before numeric to handle double-encoded API data; toast messages use decoded titles; addRemoteJobToPipeline uses displayed sort order via window._displayedJobs
