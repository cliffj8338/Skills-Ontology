...
- **CRITICAL: Module MUST override legacy**: All wizard functions in `src/views/welcome.js` use unconditional `window.X = X` (NOT `if (!window.X)`) so the module version always wins. The legacy `initializeMainApp()` in `app-core.js`/`legacy.js` must NOT re-assign wizard window globals. Additionally, ALL key wizard functions in both `app-core.js` and `legacy.js` (showOnboardingWizard, renderWizardStep, wizardNext, wizardBack, wizardStartParsing, wizardRunParsing, wizardSkipParsing, renderWizardStep5, renderWizardStep6, renderWizardStep9) have delegation guards: `if (window.X && window.X !== X) return window.X()` — this ensures that even when internal closure references call local legacy functions, they delegate to the module's window-override versions. The `renderWizardStep` switch also uses `window['renderWizardStep' + step]` to delegate individual step renderers.
- **Pilot skills from work history**: `certSkillMap` in Step 5 now also checks work history titles (not just `wizardState.certifications`) for pilot/aviation/instrument keywords. Added `pilot` key to certSkillMap alongside `private pilot` for broader matching.
- **Outcomes panel (Step 6)**: Two-list system: "Actual Outcomes" (extracted from resume evidence) + "Suggested Outcomes" (editable templates with X/Y/Z metric placeholders). More templates added: aviation, nonprofit, finance, HR categories, plus per-skill suggestions for skills without evidence.
- **Profile Strength meter (Step 9)**: Now shows a visual meter (percentage bar) + label instead of text-only "Good". Scoring based on skills count, evidence, rare/uncommon skills, outcomes, verifications. Shows actionable suggestions to boost strength.
- **currentPay flow**: `wizardSaveProfile()` → `wizardState.profile.reportedComp` → `wizardBuildUserData()` spreads → `_userData.profile.reportedComp` → Settings reads `_userData.profile.reportedComp`, Blueprint reads via `getEffectiveComp()`.
