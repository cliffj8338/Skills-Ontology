...
- Dropdowns + tracking (v4.44.85): WBW Step 1 and Converter edit form now use `<select>` dropdowns for Industry (35 options from shared `WB_INDUSTRIES[]`), Travel (`WB_TRAVEL_OPTIONS[]`), Schedule (`WB_SCHEDULE_OPTIONS[]`), and Employment Type (`WB_EMPLOYMENT_TYPES[]`). Industry includes fallback for non-matching existing values. AI_FEATURES registry extended with `wb-company-research`, `wb-value-desc`, `wb-value-desc-bulk`, `wb-jd-convert`. All WB AI calls now use correct `callAnthropicAPI(requestBody, null, featureTag)` format for proper tracking. Analytics events added: `wb_created`, `wb_edited`, `wb_deleted`, `wb_exported_pdf/word/json`, `wb_wizard_complete`, `wb_converted` via `logAnalyticsEvent()`. Admin traffic dashboard shows Work Blueprint Activity card (Row 4b) with tile counts for each WB operation when data exists
