...
## Version
Current: v4.46.3. Check `BP_VERSION` in `index.html` (line ~1647). **UNBREAKABLE RULE**: Every file change that requires a deploy MUST bump the version in ALL 3 places — even if index.html itself has no other changes: line 1 HTML comment, JS block comment, `BP_VERSION` variable.
