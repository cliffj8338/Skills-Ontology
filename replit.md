...
- Clone + Blueprint Advantage (v4.44.97): WB Repository clone feature — `wbRepoClone(idx)` deep-copies blueprint with " (Copy)" title, new ID, saves to Firestore. Blueprint Advantage comparison: live before/after matching using real engine. `_wbCompareRawJDMatch()` runs `parseJobLocally()` + `matchJobToProfile()` + `inferCompanyValuesFromJD()` on raw JD text (simulating traditional ATS keyword matching). `_wbCompareStructuredMatch()` runs structured WB skills with proficiency levels through `matchJobToProfile()` + explicit `computeValuesAlignment()`. Side-by-side visualization with SVG score gauges, matched skills tables, delta metrics (match accuracy, skills matched, values precision), and "What Work Blueprints Unlock" advantages panel. Demo candidate profile (`profiles/demo/comparison-candidate.json`) — Alex Morgan, Senior Product Strategy Manager, 24 skills with evidence, 15 values, realistic work history. Raw JD text preserved as `sourceJD` on saved WBs (was deleted as `_rawJD`). Wizard-created WBs prompt for JD paste when comparing. Firestore rules v1.4: WB key limit raised 30→50.
