...
- Find Jobs sorting + entity fix (v4.44.62–63): Sort dropdown (Match % high/low, Title A–Z, Company A–Z, Location A–Z) defaults to highest match first; sorts by displayed score (pipeline score when available, quickScore otherwise) via getJobDisplayScore(); regex-based decodeHtmlEntities for numeric/named HTML entities from API responses; addRemoteJobToPipeline uses displayed sort order via window._displayedJobs
